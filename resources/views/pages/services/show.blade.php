...
<section class="relative -mt-[100px] h-[70vh] flex items-center justify-center bg-gradient-to-b from-[#131e66] via-[#101c5c] to-[#01132f] overflow-hidden">
    ...
    <div class="relative z-10 max-w-4xl w-full mx-auto text-center px-4">
        <h1 id="service-title" data-splitting class="text-white font-extrabold text-4xl md:text-7xl leading-tight mb-6 drop-shadow-lg select-none">
            {{ $service['title'] }}
        </h1>

        @if(!empty($service['description']))
            <p id="service-subtitle" class="text-white/75 text-lg md:text-xl font-light max-w-3xl mx-auto drop-shadow-md tracking-wide opacity-0 translate-y-4">
                {{ $service['description'] }}
            </p>
        @endif
    </div>
...
<section class="relative py-24 bg-gradient-to-b from-[#f8f9fc] via-[#eaf3fe] to-[#e5e9f7] overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-6 text-slate-800 z-10 select-text">
        ...
        {{-- Intro Section: Icon & Overview --}}
        <div class="grid md:grid-cols-5 gap-12 md:gap-20 items-center mb-28">
            {{-- Service Icon --}}
            <div class="md:col-span-2 flex justify-center">
                <div class="bg-gradient-to-br from-white to-slate-200 p-12 shadow-2xl ring-1 ring-slate-200 hover:shadow-cyan-500/10 hover:shadow-2xl transition-all duration-500  transform hover:-translate-y-2">
                    <img 
                        src="{{ asset('images/services/' . $service['img']) }}" 
                        alt="{{ $service['title'] }}" 
            ...
            @if(!empty($service['details']['overview']))
                <div class="md:col-span-3 text-left">
                    <h2 class="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#2256ac] to-[#14b8a6] mb-6 pb-2">Overview</h2>
        ...
        @if(!empty($service['details']['features']))
            <section class="mb-28">
                 <h3 class="text-4xl font-bold text-slate-800 mb-12 text-center">Key Features</h3>
                <div class="max-w-6xl mx-auto grid md:grid-cols-3 gap-8 px-4">
                    @foreach($service['details']['features'] as $feature)
                        <div class="relative bg-white/60 backdrop-blur-sm border border-slate-200 shadow-lg hover:shadow-xl transition-all duration-300 p-8 flex flex-col min-h-[240px]  transform hover:-translate-y-2">
                            <img 
                                src="{{ asset('images/' . $feature['shape']) }}" 
                            ...
                            <div class="relative z-10">
                                <span class="block mb-6 p-3 bg-gradient-to-br from-[#2256ac] to-[#14b8a6] rounded-lg shadow-md w-max">
                                    <img src="{{ asset('images/' . $feature['icon']) }}" alt="Feature Icon" class="w-8 h-8 select-none invert brightness-0" />
                                </span>
        ...
        @if(!empty($service['details']['benefits']))
            <section class="bg-[#01132f] text-white -mx-6 px-6 py-20 my-28 text-center  shadow-2xl">
                <div class="max-w-4xl mx-auto">
                    <h3 class="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-white to-cyan-300 mb-5">
                        Core Benefits
                    </h3>
        ...
        @if(!empty($service['details']['use_cases']))
            <section class="mb-28">
                <h3 class="text-4xl font-bold text-slate-800 mb-12 text-center">Common Use Cases</h3>
                <div class="max-w-6xl mx-auto grid md:grid-cols-3 gap-6">
                    @foreach($service['details']['use_cases'] as $index => $useCase)
                        <div class="relative bg-slate-800 border border-slate-700 shadow-lg overflow-hidden p-8 flex items-center min-h-[120px]  transition-all duration-300 hover:border-teal-500 hover:bg-slate-700 transform hover:-translate-y-1">
                            ...
                            <div class="relative z-10 text-white text-lg font-medium">{{ $useCase }}</div>
                        </div>
                    @endforeach
                </div>
            </section>
        ...
        @if(!empty($service['details']['faq']))
            <section x-data="{ active: null }" class="mb-20 max-w-4xl mx-auto text-left">
                <h3 class="text-4xl font-bold text-slate-800 mb-12 text-center">Frequently Asked Questions</h3>
                @foreach($service['details']['faq'] as $index => $faq)
                    <div class="border border-slate-200 bg-white shadow-sm  mb-3 transition-all duration-300" :class="{ 'bg-slate-50 border-teal-300': active === {{ $index }} }">
                        <button @click="active === {{ $index }} ? active = null : active = {{ $index }}" class="w-full flex justify-between items-center py-5 px-6 text-lg font-semibold text-slate-800 focus:outline-none" type="button">
                            <span>{{ $faq['question'] }}</span>
                            <svg :class="{ 'rotate-180 text-teal-500': active === {{ $index }} }" class="w-6 h-6 transition-transform duration-300 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
                        </button>
                        <div x-show="active === {{ $index }}" x-collapse class="px-6 pb-5 border-t border-slate-200 text-slate-600 select-text" style="display:none">
...
<script type="module">
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (glowWrapper) {
            ...
            gsap.to(glowWrapper, {
                y: -120, // Increased parallax effect
                ease: 'none',
                scrollTrigger: {
                    trigger: 'section:first-of-type', // Use the hero section as trigger
                    start: 'top top',
                    end: 'bottom top',
